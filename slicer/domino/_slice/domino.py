...
from sklearn.mixture._base import check_random_state  # _check_X,
...
class DominoSlicer(Slicer):
    ...
    * Given the slice, the model predictions also vary as a categorical
    ...
    def predict_proba(
        ...
        original_result, clusters = self.mm.predict_proba(
            embeddings, y=targets, y_hat=pred_probs, test=test
        )

        return original_result, clusters[:, self.slice_cluster_indices]
...
class DominoMixture(GaussianMixture):
    ...
    def _estimate_log_prob_resp(self, X, y=None, y_hat=None, test=False):
        ...
        """
        weighted_log_prob = self._estimate_weighted_log_prob(X, y, y_hat, test)
        log_prob_norm = logsumexp(weighted_log_prob, axis=1)
    ...
    def _estimate_weighted_log_prob(self, X, y=None, y_hat=None, test=False):
        log_prob = (
            self._estimate_log_prob(X) * self.emb_log_likelihood_weight
            + self._estimate_log_weights()
        )

        if y is not None:
            y_log_prob = self._estimate_y_log_prob(y) * self.y_log_likelihood_weight
            if not test:
                log_prob += y_log_prob
            else:
                ...
                y = 1 - y
                another_y_log_prob = (
                    self._estimate_y_log_prob(y) * self.y_log_likelihood_weight
                )
                y_log_hat = np.array(
                    torch.logsumexp(
                        torch.tensor(np.array([y_log_prob, another_y_log_prob])), 0
                    )
                )
                log_prob += y_log_hat
    ...
    def _estimate_y_hat_log_prob(self, y_hat):
        ...
        # add epsilon to avoid "RuntimeWarning: divide by zero encountered in log"
        if (
            np.dot(y_hat, self.y_hat_probs.T) + np.finfo(self.y_hat_probs.dtype).eps < 0
        ).any():
            import pdb
