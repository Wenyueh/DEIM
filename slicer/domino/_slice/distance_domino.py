...
from sklearn.mixture._base import check_random_state, BaseMixture  # _check_X
...
class DistanceDominoSlicer(Slicer):
    ...
    * Given the slice, the model predictions also vary as a categorical
    ...
    def __init__(
        ...
        distance_log_likelihood_weight: float = 1,
        emb_log_likelihood_weight: float = 1,
        ...
        self.config.distance_log_likelihood_weight = distance_log_likelihood_weight
        self.config.y_hat_log_likelihood_weight = y_hat_log_likelihood_weight
        self.config.emb_log_likelihood_weight = emb_log_likelihood_weight
        ...
        self.mm = DominoMixture(
            ...
            distance_log_likelihood_weight=self.config.distance_log_likelihood_weight,
            y_hat_log_likelihood_weight=self.config.y_hat_log_likelihood_weight,
            emb_log_likelihood_weight=self.config.emb_log_likelihood_weight,
    ...
    def normalize_embedding(self, embeddings):
        # embedding_norm = 1000
        # s = np.linalg.norm(embeddings, axis=1, keepdims=True) / embedding_norm
    ...
    def predict(
        ...
        probs = self.predict_proba(
            data=data, embeddings=embeddings, targets=targets, pred_probs=pred_probs,
        )
    ...
    def predict_proba(
        ...
        weighted_log_prob, clusters = self.mm.predict_proba(
            embeddings, y=targets, y_hat=pred_probs, test=test
        )

        return weighted_log_prob, clusters

    def predict_label(self, pred_probs):
        labels = self.mm.predict_label(pred_probs)
        return labels


class DominoMixture(BaseMixture):
    @wraps(BaseMixture.__init__)
    def __init__(
        self,
        init_params,
        max_iter,
        ...
        self.embedding_gaussian = GaussianMixture(n_components=n_components,)
        self.distance_gaussian = GaussianMixture(n_components=n_components)
        self.y_hat_gaussian = GaussianMixture(n_components=n_components)
        self.distance_log_likelihood_weight = distance_log_likelihood_weight
        self.emb_log_likelihood_weight = emb_log_likelihood_weight
    ...
    def _initialize(self, X, y, y_hat, resp):
        ...
        self.embedding_gaussian.weights_ = weights
        self.embedding_gaussian.means_ = embedding_means
        self.embedding_gaussian.covariance_type = self.covariance_type

        self.distance_gaussian.weights_ = weights
        self.distance_gaussian.means_ = distance_means
        self.distance_gaussian.covariance_type = self.covariance_type

        self.y_hat_gaussian.weights_ = weights
        self.y_hat_gaussian.means_ = y_hat_means
        self.y_hat_gaussian.covariance_type = self.covariance_type

        self.embedding_gaussian.covariances = embedding_covariances
        self.embedding_gaussian.precisions_cholesky_ = _compute_precision_cholesky(
            embedding_covariances, self.covariance_type
        )
        self.distance_gaussian.covariances = distance_covariances
        self.distance_gaussian.precisions_cholesky_ = _compute_precision_cholesky(
            distance_covariances, self.covariance_type
        )
        self.y_hat_gaussian.covariances = y_hat_covariances
        self.y_hat_gaussian.precisions_cholesky_ = _compute_precision_cholesky(
            y_hat_covariances, self.covariance_type
        )
    ...
    def fit_predict(self, X, y, y_hat):
        ...
        # self.embedding_gaussian._check_n_features(X, reset=True)
        # self.embedding_gaussian._check_initial_parameters(X)
        ...
        for init in range(n_init):
            ...
            for n_iter in tqdm(
                ...
                lower_bound = (
                    log_prob_norm  # self._compute_lower_bound(log_resp, log_prob_norm)
                )
    ...
    def predict_proba(
        self, X: np.ndarray, y: np.ndarray = None, y_hat: np.ndarray = None, test=False
    ):
        y, y_hat = self._preprocess_ys(y, y_hat)

        check_is_fitted(self)
        # X = _check_X(X, None, self.embedding_gaussian.means_.shape[1])
        weighted_log_prob, log_resp = self._estimate_log_prob_resp(
            X, y, y_hat, test=test
        )
        return weighted_log_prob, np.exp(log_resp)
    ...
    def _m_step(self, X, y, y_hat, log_resp):
        ...
        (
            weights_,
            self.embedding_gaussian.means_,
            self.embedding_gaussian.covariances,
            self.distance_gaussian.means_,
            self.distance_gaussian.covariances,
            self.y_hat_gaussian.means_,
            self.y_hat_gaussian.covariances,
        ) = _estimate_parameters(
            X, y, y_hat, resp, self.reg_covar, self.covariance_type
        )

        weights_ /= weights_.sum()
        self.embedding_gaussian.weights_ = weights_
        self.distance_gaussian.weights_ = weights_
        self.y_hat_gaussian.weights_ = weights_

        self.embedding_gaussian.precisions_cholesky_ = _compute_precision_cholesky(
            self.embedding_gaussian.covariances, self.covariance_type
        )
        self.distance_gaussian.precisions_cholesky_ = _compute_precision_cholesky(
            self.distance_gaussian.covariances, self.covariance_type
        )
        self.y_hat_gaussian.precisions_cholesky_ = _compute_precision_cholesky(
            self.y_hat_gaussian.covariances, self.covariance_type
        )
    ...
    def _e_step(self, X, y, y_hat):
        ...
        log_responsibility : array, shape (n_samples, n_components)
            ...
            the point of each sample in X.
        """
        weighted_log_prob, log_resp = self._estimate_log_prob_resp(X, y, y_hat)
    ...
    def _estimate_log_prob_resp(self, X, y=None, y_hat=None, test=False):
        ...
        """
        weighted_log_prob = self._estimate_weighted_log_prob(X, y, y_hat, test=test)
        log_prob_norm = logsumexp(weighted_log_prob, axis=1)
        with np.errstate(under="ignore"):
            # ignore underflow
            log_resp = weighted_log_prob - log_prob_norm[:, np.newaxis]
        return weighted_log_prob, log_resp

    def compute_complement_label(self, pseudo_y, i):
        n = len(pseudo_y[0])
        indices = [[i for i, v in enumerate(y) if v == 1] for y in pseudo_y]
        returned_y = []
        for j in range(len(pseudo_y)):
            one_y = [0] * n
            one_y[indices[j][i]] = 1
            returned_y.append(one_y)
        return np.array(returned_y)

    def _estimate_weighted_log_prob(self, X, y=None, y_hat=None, test=False):
        # n_components
        weight_log_prob = self._estimate_log_weights(self.embedding_gaussian)

        # n * n_componenets
        log_prob = (
            self._estimate_embedding_log_prob(self.embedding_gaussian, X)
            * self.emb_log_likelihood_weight
            + weight_log_prob
        )
        ...
        if y is not None and y_hat is not None:
            distance = y - y_hat
            distance_log_prob = (
                self._estimate_distance_log_prob(self.distance_gaussian, distance)
                * self.distance_log_likelihood_weight
            )
            if not test:
                log_prob += distance_log_prob
            else:
                marginalize = [distance_log_prob]
                print("compute marginalization")
                pseudo_y = 1 - y
                for i in range(len(y[0]) - 1):
                    one_y = self.compute_complement_label(pseudo_y, i)
                    distance = one_y - y_hat
                    another_distance_log_prob = (
                        self._estimate_distance_log_prob(
                            self.distance_gaussian, distance
                        )
                        * self.distance_log_likelihood_weight
                    )
                    marginalize.append(another_distance_log_prob)
                distance_log_prob = np.array(
                    torch.logsumexp(torch.tensor(np.array(marginalize)), 0,)
                )
                log_prob += distance_log_prob

            y_hat_log_prob = (
                self._estimate_y_hat_log_prob(self.y_hat_gaussian, y_hat)
                * self.y_hat_log_likelihood_weight
            )

            log_prob += y_hat_log_prob

        return log_prob

    def _estimate_log_weights(self, gaussian_model):
        return np.log(gaussian_model.weights_)

    def _estimate_embedding_log_prob(self, gaussian_model, X):
        log_prob = _estimate_log_gaussian_prob(
            X,
            gaussian_model.means_,
            gaussian_model.precisions_cholesky_,
            gaussian_model.covariance_type,
        )

        return log_prob

    def _estimate_distance_log_prob(self, gaussian_model, distance):
        log_prob = _estimate_log_gaussian_prob(
            distance,
            gaussian_model.means_,
            gaussian_model.precisions_cholesky_,
    ...
    def _estimate_y_hat_log_prob(self, gaussian_model, y_hat):
        log_prob = _estimate_log_gaussian_prob(
            y_hat,
            gaussian_model.means_,
            gaussian_model.precisions_cholesky_,
    ...
    def _get_parameters(self):
        return (
            self.embedding_gaussian.weights_,
            self.distance_gaussian.weights_,
            self.y_hat_gaussian.weights_,
            self.embedding_gaussian.means_,
            self.embedding_gaussian.covariances,
            self.distance_gaussian.means_,
            self.distance_gaussian.covariances,
            self.y_hat_gaussian.means_,
            self.y_hat_gaussian.covariances,
            self.embedding_gaussian.precisions_cholesky_,
            self.distance_gaussian.precisions_cholesky_,
            self.y_hat_gaussian.precisions_cholesky_,
        )
    ...
    def _set_parameters(self, params):
        ...
        if self.covariance_type == "full":
            self.embedding_gaussian.precisions_ = np.empty(
                self.embedding_gaussian.precisions_cholesky_.shape
            )
            for k, prec_chol in enumerate(self.embedding_gaussian.precisions_cholesky_):
                self.embedding_gaussian.precisions_cholesky_[k] = np.dot(
                    prec_chol, prec_chol.T
                )

            self.distance_gaussian.precisions_ = np.empty(
                self.distance_gaussian.precisions_cholesky_.shape
            )
            for k, prec_chol in enumerate(self.distance_gaussian.precisions_cholesky_):
                self.distance_gaussian.precisions_cholesky_[k] = np.dot(
                    prec_chol, prec_chol.T
                )

            self.y_hat_gaussian.precisions_ = np.empty(
                self.y_hat_gaussian.precisions_cholesky_.shape
            )
            for k, prec_chol in enumerate(self.y_hat_gaussian.precisions_cholesky_):
                self.y_hat_gaussian.precisions_cholesky_[k] = np.dot(
                    prec_chol, prec_chol.T
                )

        elif self.covariance_type == "tied":
            self.embedding_gaussian.precisions_ = np.dot(
                self.embedding_gaussian.precisions_cholesky_,
                self.embedding_gaussian.precisions_cholesky_.T,
            )
            self.distance_gaussian.precisions_ = np.dot(
                self.distance_gaussian.precisions_cholesky_,
                self.distance_gaussian.precisions_cholesky_.T,
            )
            self.y_hat_gaussian.precisions_ = np.dot(
                self.y_hat_gaussian.precisions_cholesky_,
                self.distance_gaussian.precisions_cholesky_.T,
            )
        else:
            self.embedding_gaussian.precisions_ = (
                self.embedding_gaussian.precisions_cholesky_ ** 2
            )
            self.distance_gaussian.precisions_ = (
                self.distance_gaussian.precisions_cholesky_ ** 2
            )
            self.y_hat_gaussian.precisions_ = (
                self.y_hat_gaussian.precisions_cholesky_ ** 2
            )
...
def _estimate_parameters(X, y, y_hat, resp, reg_covar, covariance_type):
    ...
    embedding_means = np.dot(resp.T, X) / nk[:, np.newaxis]
    embedding_covariances = {
        "full": _estimate_gaussian_covariances_full,
        "tied": _estimate_gaussian_covariances_tied,
        "diag": _estimate_gaussian_covariances_diag,
        "spherical": _estimate_gaussian_covariances_spherical,
    }[covariance_type](resp, X, nk, embedding_means, reg_covar)

    distance = y - y_hat

    # n_components * n_classes
    distance_means = np.dot(resp.T, distance) / nk[:, np.newaxis]
    distance_covariances = {
        "full": _estimate_gaussian_covariances_full,
        "tied": _estimate_gaussian_covariances_tied,
        "diag": _estimate_gaussian_covariances_diag,
        "spherical": _estimate_gaussian_covariances_spherical,
    }[covariance_type](resp, distance, nk, distance_means, reg_covar)

    # n_components * n_classes
    y_hat_means = np.dot(resp.T, y_hat) / nk[:, np.newaxis]
    y_hat_covariances = {
        "full": _estimate_gaussian_covariances_full,
        "tied": _estimate_gaussian_covariances_tied,
        "diag": _estimate_gaussian_covariances_diag,
        "spherical": _estimate_gaussian_covariances_spherical,
    }[covariance_type](resp, y_hat, nk, y_hat_means, reg_covar)
